...
Load this reference whenever a rendered sequence needs a custom subject, texture, environment, background plate, collage element, raster illustration, or transparent cutout.

## 1. Write the asset contract

Before choosing a provider, define only what production needs:

- narrative role, scene, beat, and maximum on-screen size;
- asset type and whether it will move, scale, rotate, blur, or cross the camera;
- canvas dimensions, ratio, resolution, overscan, crop, and required variants;
- silhouette, perspective, lens feeling, lighting, material, palette, and negative space;
- relationship to official identity and reference images;
...
- final PNG, WebP, SVG, layered source, or image-sequence format.

Never regenerate an official logo. Keep critical typography editable whenever timing, localization, or revision matters.
...
7. local SVG, Canvas, Three.js, shader, procedural texture, or accessible stock;
...
Resolve the installed `balise-motion-graphic` directory first; do not assume the project working directory contains the skill. Then run:

```bash
python <balise-motion-graphic-skill-dir>/scripts/setup_asset_pipeline.py --check --backend auto --scope user
```

When production or workflow setup authorizes reversible user-level installation:

```bash
python <balise-motion-graphic-skill-dir>/scripts/setup_asset_pipeline.py --install --backend pillow --scope user
```
...
5. inspect it in a representative hero frame;
...
4. Place it in a hero frame at maximum expected scale.
...
```bash
python <balise-motion-graphic-skill-dir>/scripts/remove_chroma_key.py \
  --input source.png \
  --output cutout.png \
  --key auto-border \
  --backend auto \
...
## 7. Render and compositing checks

- Validate alpha over checkerboard, light, dark, and scene-colored plates.
- Inspect fringes, internal holes, subject detail, and premultiplication.
- Preserve an unpremultiplied alpha master unless the compositor requires otherwise.
- Generate enough overscan for camera movement, parallax, blur, rotation, and scale.
- Test the asset at the largest delivered frame and at dense transition frames.
- Keep lighting, perspective, grain, sharpness, and color response consistent across a series.
- Upscale before final cutout when enlargement would otherwise damage the matte; recheck edges afterward.
- Confirm that the renderer and codec preserve alpha when alpha delivery is required.
- Re-render and repeat proof whenever the asset, crop, color treatment, or source file changes.
